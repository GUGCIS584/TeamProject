...
$result1=mysqli_query($con,"select count(*) count from facultydetails");
...
mysqli_query($con, '
    LOAD DATA LOCAL INFILE "../csv/'.$filename.'"
        INTO TABLE facultydetails
        FIELDS TERMINATED by \',\'
		OPTIONALLY ENCLOSED BY \'"\'
        LINES TERMINATED BY \'\n\'
		IGNORE 1 LINES
		(name, department, typeofjob, subject_total, subject1, subject2, subject3, subject4, subject5, yearly_salary, pay_per_subject, hours, faculty_total)
')or die(mysql_error());
}
else{
	mysqli_query($con, '
...
$result2=mysqli_query($con,"select count(*) count from facultydetails");
